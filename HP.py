...
def merge_pictures(player_A, player_B):
    ...
    img_lightning_bolt = img_lightning_bolt.resize(
        (round(width_img_lightning_bolt/10), round(height_img_lightning_bolt/10)))
    width_img_lightning_bolt, height_img_lightning_bolt = img_lightning_bolt.size
    new_image.paste(img_lightning_bolt, (width -
                    (round(width_img_lightning_bolt/2)), round(height/5)))
...
def play(player_A, player_B, score_record):
    if (player_A['wizard'] is False) and (player_B['wizard'] is False):
        ...
        while continue_round is True:
            player_A['life_point'], player_B['life_point'] = attack(
                player_A, player_B)
            continue_round = evaluate_life_point(
                player_A, player_B, score_record)
...
def evaluate_life_point(player_A, player_B, score_record):
    if (player_A['life_point'] <= 0) and (player_B['life_point'] > 0):
        print("Player B win")
        score_record.loc[score_record["name"]
                         == player_B['name'], "score"] += 10
...
def cast_a_spell(player_attacking, player_attacked):
    ...
    player_attacked['life_point'] = calculate_damage(
        spell_to_cast, player_attacked, player_attacking)
...
def computer_cast_spell(player_attacked, player_attacking):
    ...
    player_attacked['life_point'] = calculate_damage(
        attack, player_attacked, player_attacking)
    return player_attacked['life_point']


def calculate_damage(attack, player_attacked, player_attacking):
    player_attacked['life_point'] = player_attacked['life_point'] - \
        player_attacking['spells'][attack]
    return player_attacked['life_point']
